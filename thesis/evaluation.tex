...
% TODO
% key finding: latency of replica ack matters a lot
% in the 10 us case, if the replica runs the synthetic workload in the same
% thread as raft, very low throughput
% We then patched the algorithm to have the replicas not run the workload
% (simulating worker thread as far as Raft is concerned)
% much better throughput

\begin{figure}[p]
    \centering
    \begin{subfigure}[t]{0.8\textwidth}
        \includegraphics[width=\textwidth]{plots/latency_10us_synthetic}
        \caption{\SI{10}{\micro\second} work}
    \end{subfigure}

    \begin{subfigure}[t]{0.8\textwidth}
        \includegraphics[width=\textwidth]{plots/latency_0us_synthetic}
